...
/* The visor hood is an open cylinder that must render from both sides.
   Setting .side on MAT.steelDark would flip every steelDark surface in the
   world to DoubleSide — a shared-material mutation, the exact class of bug
   the material freeze exists to catch. One dedicated clone instead. */
let steelDarkDS = null;
function visorMat() {
  if (!steelDarkDS) {
    steelDarkDS = MAT.steelDark.clone();
    steelDarkDS.name = 'steelDarkVisor';
    steelDarkDS.side = THREE.DoubleSide;
  }
  return steelDarkDS;
}
 ...
 */
export function signalHead(sections = 3, lensMats) {
  ...
  /* Cloned per CONTROLLER LEG, never shared world-wide. One material for the
     whole world would light every red lens at every intersection in unison —
     the defect v4 fixed in the building windows. But per-HEAD clones (v4) put
     624 unmergeable lens meshes in the scene; every head on the same leg of
     the same intersection shows the same state at the same instant, so one
     clone per leg per colour drives them all and lets the merge pass collapse
     the meshes. The caller passes the shared set in; standalone use clones. */
  lensMats = lensMats ||
    [MAT.lensRed.clone(), MAT.lensYellow.clone(), MAT.lensGreen.clone()];
  const lenses = [];
...
/* ------------------------------------------------------- pedestrian signal */
export function pedSignalHead(pedMats) {
  const g = new THREE.Group();
  g.name = 'ped-signal';
  ...
  /* cloned per controller leg, for the same reason the vehicle lenses are */
  pedMats = pedMats || { hand: MAT.lensPedHand.clone(), walk: MAT.lensPedWalk.clone(),
                         count: MAT.lensPedHand.clone() };
  const hand = mesh(box(w * 0.42, PED_SIGNAL.symbolHeight, IN(0.4)),
    pedMats.hand, -w * 0.22, h * 0.12, d / 2 + IN(0.2), { cast: false, receive: false });
  const walk = mesh(box(w * 0.42, PED_SIGNAL.symbolHeight, IN(0.4)),
    pedMats.walk, w * 0.22, h * 0.12, d / 2 + IN(0.2), { cast: false, receive: false });
  const count = mesh(box(w * 0.5, PED_SIGNAL.countdownDigitHeight, IN(0.4)),
    pedMats.count, 0, -h * 0.3, d / 2 + IN(0.2), { cast: false, receive: false });
...
export function signalMast(x, z, armLength, armAzimuth, faceAzimuth, opts = {}) {
  ...
  for (const d of positions) {
    const head = signalHead(3, opts.lensMats);
  ...
  const near = signalHead(3, opts.lensMats);
  ...
  if (opts.pedestrian !== false) {
    const ped = pedSignalHead(opts.pedMats);
