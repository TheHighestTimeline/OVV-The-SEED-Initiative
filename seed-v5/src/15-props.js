  ...
  placeContainer({
    ...
    build: () => {
      ...
      for (const rt of routes) {
        ...
        const c = car(0, 0, 0, r(), { mobile: true });
...
function car(x, z, rot, seed, opts = {}) {
  const g = new THREE.Group();
  const y = groundH(x, z);
  ...
  /* Only a MOVING car must stay unmerged — bake a driving car into a static
     batch and it freezes where it was built while its group animates empty.
     The flag goes on the GROUP, which the merge passes prune as a subtree;
     the ~90 parked cars stay mergeable down to a few draws. */
  if (opts.mobile) g.userData.noMerge = true;
  g.add(body, cabin);
...
function shuttle() {
  const g = new THREE.Group();
  g.userData.noMerge = true;   /* it drives — see car() */
