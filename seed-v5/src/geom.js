...
export function mergeStatic(group, opts) {
  ...
  const keep = [];
  /* Manual walk instead of traverse so a noMerge GROUP prunes its whole
     subtree. The mesh-level check alone was a real shipped bug: a driving
     car's group is animated but its body and tyres carried no flag, so the
     merge baked them into a static batch and the only thing left driving
     was the cabin — a ghost glass box on the arterial. */
  (function walk(o) {
    if (o.userData && o.userData.noMerge) return;
    if (o.isMesh && !o.isInstancedMesh) {
      /* InstancedMesh extends Mesh. Merging one would bake a single instance
         and silently delete every other copy — the wrack line, the oyster
         beds, the sand fence and the plantings all live in instanced meshes. */
      if (o.geometry.attributes.position.count > (opts.maxVerts || 60000)) {
        keep.push(o);
      } else {
...
/* ------------------------------------------------- instanced-mesh collapse */
/**
 * Merge InstancedMesh objects that draw the same thing into one draw.
 *
 * The generators are honest about instancing — every dome pad, cobble run and
 * PV array is an InstancedMesh — but they instance per CALL SITE, so 124
 * curb-ramp pads each carry their own 25-dome batch and the bioswale carries
 * 111 separate cobble batches. Identical parametric geometries are re-created
 * per call (`new DodecahedronGeometry(0.42, 0)` at every check dam), so
 * bucketing by geometry uuid would merge nothing: the bucket key is the
 * geometry's parameter signature instead, plus the material and the render
 * flags. Instance matrices are composed into world space, so the merged mesh
 * lives at the group root with no transform of its own.
 *
 * Skips userData.noMerge. Never bakes: instances stay instances, so the
 * per-instance wind shader and instance colours keep working.
 */
export function collapseInstanced(group) {
  group.updateWorldMatrix(true, true);
  const buckets = new Map();
  const all = [];
  (function walk(o) {
    if (o.userData && o.userData.noMerge) return;   /* prunes whole subtree */
    if (o.isInstancedMesh) all.push(o);
    for (const c of o.children) walk(c);
  })(group);
  for (const o of all) {
    const g = o.geometry;
    const sig = g.parameters
      ? g.type + JSON.stringify(g.parameters)
      : g.uuid;
    const key = [sig, o.material.uuid, o.renderOrder,
  ...
  for (const list of buckets.values()) {
    ...
    const total = list.reduce((n, o) => n + o.count, 0);
    const first = list[0];
    const im = new THREE.InstancedMesh(first.geometry, first.material, total);
    im.name = first.name || 'inst-merged';
    im.castShadow = first.castShadow;
    im.receiveShadow = first.receiveShadow;
    im.renderOrder = first.renderOrder;
    const hasColor = list.some((o) => o.instanceColor);
    let w = 0;
    for (const o of list) {
      o.updateWorldMatrix(true, false);
      tmp.multiplyMatrices(groupInv, o.matrixWorld);
      for (let i = 0; i < o.count; i++) {
        o.getMatrixAt(i, inst);
        inst.premultiply(tmp);
        im.setMatrixAt(w, inst);
        if (hasColor) {
          if (o.instanceColor) o.getColorAt(i, col); else col.setRGB(1, 1, 1);
          im.setColorAt(w, col);
        }
        w++;
      }
    ...
    /* frustum culling on an InstancedMesh uses its instance-aware bounding
       sphere (r156+), so a world-spanning batch still culls correctly */
