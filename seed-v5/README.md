...
`npm ci && npm run build` in `seed-v5/` and publish `dist/`. Hashed assets are
...
There is no time scrubber. The world has exactly two states — **Afternoon**
(sun at ~27°, long soft shadows) and **Night** (street lights on, per-window
lit patterns, the dark-sky eastern zone actually dark) — toggled from the
bottom bar. Both run through the same physical sky pipeline; each state's
PMREM environment map and fog probe are baked on first visit and cached, so
the toggle is a hard swap with no rebake. The cinematic tour plays the campus
and the town in the afternoon and switches to night at the coast.
...
`__seedTest()` · `__seedTime("afternoon"|"night")` · `__seedView(i)` · `__seedWalk(x, z, heading)` ·
`__seedPlayer()`
