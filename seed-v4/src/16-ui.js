...
import { SITE, TIER_ORDER, clamp, lerp, smoothstep } from './00-config.js';
...
/* the cinematic tour: eight zones, eased, with a day-to-night arrival */
const TOUR = [
  { view: 0, hold: 5.0, time: 9.4, open: null },
  { view: 1, hold: 5.5, time: 11.0, open: 'i1' },
  { view: 2, hold: 5.5, time: 13.2, open: 'm3' },
  { view: 3, hold: 6.0, time: 15.6, open: 'm1' },
  { view: 4, hold: 5.5, time: 17.4, open: 'b9' },
  { view: 5, hold: 5.5, time: 18.4, open: 'h5' },
  { view: 6, hold: 6.5, time: 19.3, open: 'h1' },
  { view: 7, hold: 8.0, time: 20.6, open: 'h4' },
];
...
export function initUI(state) {
  ...
  let timePending = null;
  timeEl.oninput = () => {
    stopTour();
    const h = parseFloat(timeEl.value);
    clockEl.textContent = fmt(h);
    timePending = h;
  };
  document.querySelectorAll('#presets button').forEach((b) => {
    b.onclick = () => {
      ...
      timeEl.value = h; clockEl.textContent = fmt(h); timePending = h;
    };
  });
  ...
  function startTour() {
    ...
    state.tour = {
      update(dt) {
        ...
        const target = leg.time;
        if (Math.abs(pipeline.hour - target) > 0.02) {
          pipeline.setTimeOfDay(lerp(pipeline.hour, target, 0.012));
          timeEl.value = pipeline.hour;
          clockEl.textContent = fmt(pipeline.hour);
        }
  ...
  state.onFrame = (dt, now) => {
    clock += dt;
    if (timePending != null) { pipeline.setTimeOfDay(timePending); timePending = null; }
