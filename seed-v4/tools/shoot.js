...
const file = path.join(root, 'dist',
  `OVMG_SEED_3DCampusWorld_20260809_v4${dev ? '_dev' : ''}.html`);
...
if (ready) {
  ...
  if (doShots) {
    ...
    const times = [{ h: 13.0, n: 'midday' }, { h: 19.1, n: 'golden' }, { h: 21.6, n: 'night' }];
    for (let i = 0; i < views.length; i++) {
      for (const t of times) {
        await page.evaluate(([vi, hh]) => { window.__seedView(vi); window.__seedTime(hh); },
          [i, t.h]);
        await page.waitForTimeout(1400);
        await page.screenshot({ path: path.join(dir, `${String(i).padStart(2, '0')}_${views[i]}_${t.n}.png`) });
      }
    }
